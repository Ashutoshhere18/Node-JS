...
export default function Blog(){
  ...
  const getBlogs = async () => {
    const res = await axios.get("http://localhost:4050/getBlogs",{withCredentials:true});
  ...
  const handlePublish = async () => {
    try{
      ...
      if(editId){
        res = await axios.put(
          `http://localhost:4050/updateBlogs/${editId}`,
          formData,
          { withCredentials:true }
        );
      }else{
        res = await axios.post(
          "http://localhost:4050/createBlog",
          formData,
          { withCredentials:true }
        );
      }
  ...
  const handleDelete = async (id) => {
    ...
    const res = await axios.delete(
      `http://localhost:4050/deleteBlogs/${id}`,
      { withCredentials:true }
    );
  ...
  return (
    <div className="auth-wrapper">
      <div className="auth-card" style={{width:"900px"}}>
        ...
        <div className="brand">Dev<span>Sphere</span> Blog</div>
        <div className="subtitle">Secure SaaS Blogging Platform 🚀</div>
        ...
        {blogs.map((b)=>(
          <div key={b._id} style={{
            ...
            {b.image && (
              <img 
                src={`http://localhost:4050/uploads/${b.image}`} 
                alt="" 
                style={{
                  width:"100%",
                  borderRadius:"12px",
